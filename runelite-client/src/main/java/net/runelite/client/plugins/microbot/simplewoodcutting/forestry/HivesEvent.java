...
public class HivesEvent implements BlockingEvent {
    ...
    public HivesEvent(SimpleWoodcuttingPlugin plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean validate() {
        try{
            ...
            return !beehives.isEmpty() && tree != null
                    && Rs2Inventory.count(tree.getLogName()) > 1;
        } catch (Exception e) {
    ...
    public boolean execute() {
        ...
        TreeStage tree = plugin.getSelectedTree();
        initialLogCount = tree == null ? 0 : Rs2Inventory.count(tree.getLogName());
        ...
        while (this.validate()) {
            ...
            if (Rs2Player.isInteracting() || Rs2Player.isAnimating()) {
                ...
                boolean completed = sleepUntil(() -> plugin.isScriptPaused()
                        || !Rs2Player.isInteracting() && !Rs2Player.isAnimating(), 30000);
                if (plugin.isScriptPaused()) {
                    return true;
                }
            ...
            TreeStage treeType = plugin.getSelectedTree();
            int currentLogCount = treeType == null
                    ? 0
                    : Rs2Inventory.count(treeType.getLogName());
            ...
            if (targetBeehive.click("Build")) {
                ...
                boolean buildCompleted = sleepUntil(() -> plugin.isScriptPaused()
                        || !Rs2Player.isInteracting() && !Rs2Player.isAnimating(), 15000);
                if (plugin.isScriptPaused()) {
                    return true;
                }

                if (buildCompleted) {
                    int newLogCount = treeType == null
                            ? 0
                            : Rs2Inventory.count(treeType.getLogName());
        ...
        TreeStage finalTree = plugin.getSelectedTree();
        int finalLogCount = finalTree == null
                ? 0
                : Rs2Inventory.count(finalTree.getLogName());
