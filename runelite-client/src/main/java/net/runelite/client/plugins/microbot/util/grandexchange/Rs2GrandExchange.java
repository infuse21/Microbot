...
//					MenuEntryImpl(getOption=Customise, getTarget=, getIdentifier=2, getType=CC_OP, getParam0=15, getParam1=30474266, getItemId=-1, isForceLeftClick=false, getWorldViewId=-1, isDeprioritized=false)
    ...
    private static boolean setQuantity(int quantity) {
        boolean success = retryQuantity(quantity, Rs2GrandExchange::getOfferQuantity, () -> {
            ...
            Rectangle quantityBounds = Microbot.getClientThread()
                    .runOnClientThreadOptional(quantityButtonX::getBounds).orElse(null);
            if (quantityBounds == null) return;
            Microbot.getMouse().click(quantityBounds);
