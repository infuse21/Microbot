...
public class RemainingTransportClassificationTest
{
	...
	public void pinsCurrentLegacyClassificationBoundary()
	{
		...
		expected.put(TransportType.TRANSPORT, 542);
		expected.put(TransportType.TELEPORTATION_ITEM, 37);
		assertEquals(expected, byType);
		assertEquals(579, byType.values().stream().mapToInt(Integer::intValue).sum());
	}
}
